...
def center_of_mass(img,border=0,thresh=0):
    """
    ...
    if border:
        brd = int(border)
        border = np.ones(regs.shape,np.bool)
        border[brd:-brd,brd:-brd] = 0
#        border[-brd:,:]=border[:brd,:]=border[:,-brd:]=border[:,:brd] = 1
    ...
    for id in np.unique(regs[regs>thresh]):
        cms_tmp = cms*0
        cms_tmp[regs==id] = 1
        ym,xm = ndi.center_of_mass(cms_tmp)
        if border[ym,xm]: continue
        cms[ym,xm] = id
