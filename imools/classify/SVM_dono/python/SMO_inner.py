...
def simple( C, tol, max_passes, X_tr, Y_tr ):
    ...
    if (num_params < 2) :
        print >> sys.stderr,"Error: Wrong training set.";
        return (False);
    ...
    while (passes < max_passes):
        ...
        for i in range( 0, num_exples ):
            ...
            if (( y_i*E_i < -1*tol and alpha_i < C ) or ( y_i*E_i > tol and alpha_i > 0 )):
                ...
                alpha[j] = H if alpha_j > H else L if alpha_j < L else alpha_j;
...
#                    continue;
                ...
                beta = b1 if ( 0 < alpha_i < C ) else b2 if ( 0 < alpha_j < C ) else (b1+b2)/2;
