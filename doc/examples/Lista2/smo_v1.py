...
def E_deviation( alpha, b, X, Y, i, m):

    return ( b - Y[i] + sum([ alpha[k] * Y[k] * np.dot(X[k].T,X[i]) for k in range(0,m) ]) );
...
#            E[i] = b - Y[i] + sum([ alpha[k] * Y[k] * np.dot(X[k].T,X[i]) for k in range(0,m) ]);
            E[i] = E_deviation( alpha, b, X, Y, i, m);
...
#                E[j] = -Y[j] + b + sum([ alpha[k] * Y[k] * np.dot(X[k].T,X[j]) for k in range(0,m) ]);
                E[j] = E_deviation( alpha, b, X, Y, j, m);
